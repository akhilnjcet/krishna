...
const SupportHub = () => {
    ...
    useEffect(() => {
        ...
        if (role === 'admin') {
            ...
            // User requested "Staff can seperate chat for admin... based on project"
            // Let's allow staff to see ALL support chats IF they aren't the requester, 
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-700">
            <header className="text-center space-y-2 mb-12">
                ...
                <h1 className="text-4xl font-black text-slate-800 dark:text-slate-200 tracking-tighter uppercase italic">Communication Hub</h1>
                <p className="text-slate-500 dark:text-slate-400 font-bold tracking-widest uppercase text-[10px]">Secure End-to-End Encrypted Tunnel</p>
            ...
            <div className="flex items-center gap-6 border-b-2 border-slate-100 dark:border-slate-800 mb-10">
                <button 
                    onClick={() => setActiveTab('active')}
                    className={`pb-4 px-4 text-xs font-black uppercase tracking-[0.2em] transition-all relative ${
                        activeTab === 'active' ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600 dark:text-slate-400'
                    }`}
                >
                ...
                <button 
                    ...
                    className={`pb-4 px-4 text-xs font-black uppercase tracking-[0.2em] transition-all relative ${
                        activeTab === 'closed' ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600 dark:text-slate-400'
                    }`}
                >
                    ...
                    {(activeTab === 'active' ? activeTickets : closedTickets)
                        ...
                        <div 
                            ...
                            className={`p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] border-4 transition-all relative overflow-hidden flex flex-col justify-between h-full group ${
                                ...
                                ? 'bg-white dark:bg-slate-900 border-indigo-600 shadow-[10px_10px_0px_0px_#4f46e5]' 
                                : ticket.status === 'closed' 
                                ? 'bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 opacity-60' 
                                : 'bg-white dark:bg-slate-900 border-slate-900 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]'
                            }`}
                        >
                            <div className="relative z-10">
                                ...
                                <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-2 uppercase tracking-tight">{ticket.projectTitle}</h3>
                                <div className="flex items-center gap-2 mb-6">
                                    ...
                                    {ticket.requestType === 'staff-reference' && (
                                        <span className="bg-amber-100 text-amber-700 text-[9px] font-black px-2 py-1 rounded">INTERNAL</span>
                                    )}
                                    <span className="text-[10px] text-indigo-600 font-bold uppercase tracking-widest">{ticket.reason}</span>
                                </div>
                                
                                <div className="bg-slate-50 dark:bg-slate-800 p-6 rounded-2xl border-2 border-slate-100 dark:border-slate-800 mb-8 max-h-32 overflow-hidden relative">
                            ...
                            <div className="flex gap-4 pt-8 border-t-2 border-dashed border-slate-100 dark:border-slate-800 relative z-10">
                                {ticket.status === 'approved' ? (
                                    <>
                                        <button 
                                    ...
                                    role === 'admin' ? (
                                        <div className="flex gap-4 w-full">
                                            <button 
                                                ...
                                                className="flex-1 bg-slate-50 dark:bg-slate-800 hover:bg-rose-50 text-slate-400 hover:text-rose-600 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all border border-slate-200 dark:border-slate-700"
                                            >
            ...
            {!loading && (activeTab === 'active' ? activeTickets : closedTickets).length === 0 && (
                <div className="py-32 text-center bg-white dark:bg-slate-900 rounded-[4rem] border-4 border-dashed border-slate-100 dark:border-slate-800">
