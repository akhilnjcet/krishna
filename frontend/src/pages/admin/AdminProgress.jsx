    ...
    return (
        <div className="p-4 md:p-8 space-y-8 bg-slate-50/50/50 dark:bg-slate-800/50 min-h-screen font-sans">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <h1 className="text-4xl font-black text-slate-900 dark:text-white italic tracking-tighter uppercase flex items-center gap-3">
                        <BarChart className="w-10 h-10 text-indigo-600" />
                        PROD. MONITORING
                    </h1>
                    <p className="text-slate-500 dark:text-slate-400 font-bold uppercase tracking-[0.2em] text-[10px] mt-1 ml-1">Work Site Operational Logs & Progress Tracking</p>
                </div>

                <div className="flex gap-4">
                    <div className="bg-white dark:bg-slate-900 border-2 border-slate-900 p-4 shadow-custom flex flex-col items-center min-w-[120px]">
            ...
            {/* Filter Bar */}
            <div className="flex flex-wrap gap-4 items-center bg-white dark:bg-slate-900 p-6 border-4 border-slate-900 shadow-custom">
                ...
                {['all', 'In Progress', 'Completed', 'Not Started'].map(f => (
                    <button
                        ...
                        className={`px-6 py-2 font-black uppercase tracking-widest text-[10px] border-2 transition-all ${
                            filter === f ? 'bg-indigo-600 text-white border-slate-900' : 'bg-white dark:bg-slate-900 text-slate-400 border-slate-200 dark:border-slate-700 hover:border-slate-400'
                        }`}
                    >
                        {f}
                    </button>
            ...
            {loading ? (
                ...
                <div className="bg-white dark:bg-slate-900 border-4 border-dashed border-slate-200 dark:border-slate-700 p-20 rounded-[2.5rem] text-center">
                ...
                <div className="space-y-6">
                    {filteredReports.map((report, idx) => (
                        <motion.div 
                            ...
                            className="bg-white dark:bg-slate-900 border-4 border-slate-900 overflow-hidden shadow-custom"
                        >
                            <div className="flex flex-col md:flex-row items-stretch">
                                ...
                                <div className="flex-grow p-6">
                                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                                        <div>
                                            ...
                                            <div className="flex items-center gap-4 text-[10px] font-black tracking-widest uppercase text-slate-400">
                                                <span className="flex items-center gap-1 text-slate-900 dark:text-white italic"><User className="w-3 h-3 text-indigo-600" /> {report.staffId?.name || 'Staff N/A'}</span>
                                    ...
                                    <div className="bg-slate-50 dark:bg-slate-800 border-L-4 border-indigo-600 p-4 mb-4">
                                        <h4 className="text-xs font-black uppercase text-indigo-600 mb-2 tracking-widest">{report.title}</h4>
                                        <p className="text-sm font-medium text-slate-700 dark:text-slate-300 leading-relaxed">{report.description}</p>
                                    </div>
                                    ...
                                    <AnimatePresence>
                                        {expandedId === report._id && (
                                            <motion.div 
                                                ...
                                                className="overflow-hidden border-t-2 border-slate-100 dark:border-slate-800 mt-6 pt-6"
                                            >
                                                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                                    <div className="space-y-4">
                                                        <div>
                                                            ...
                                                            <p className="text-xs font-bold text-slate-900 dark:text-white bg-white dark:bg-slate-900 p-3 border-2 border-slate-100 dark:border-slate-800 italic">{report.materialsUsed || 'None listed'}</p>
                                                        </div>
                                                        <div>
                                                            ...
                                                            <p className="text-xs font-bold text-slate-900 dark:text-white bg-red-50 p-3 border-2 border-red-100">{report.issues || 'No issues reported'}</p>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="space-y-4">
                                                        <div>
                                                            ...
                                                            <p className="text-xs font-bold text-slate-900 dark:text-white bg-indigo-50 p-3 border-2 border-indigo-100">{report.nextPlan || 'Awaiting schedule'}</p>
                                                        </div>
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="grid grid-cols-3 gap-2">
                                                            {report.photos?.map((photo, pIdx) => (
                                                                <a key={pIdx} href={photo.url} target="_blank" rel="noreferrer" className="aspect-square border-2 border-slate-200 dark:border-slate-700 hover:border-indigo-600 transition-colors cursor-zoom-in group relative overflow-hidden">
                                                            ...
                                                            {(!report.photos || report.photos.length === 0) && (
                                                                <div className="col-span-3 py-6 bg-slate-50 dark:bg-slate-800 border-2 border-dashed border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center">
