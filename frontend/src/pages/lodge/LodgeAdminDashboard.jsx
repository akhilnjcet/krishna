...
const LodgeAdminDashboard = () => {
    ...
    // Standardized Working Navigation Modules
    const tabs = [
        ...
        { id: 'customers', label: 'Customers', icon: History },
        { id: 'settings', label: 'Settings', icon: Settings },
    ];

    // Real-Time Socket / Store Auto-Sync
    React.useEffect(() => {
        pullFromCloud();
        fetchAllBookings();
        fetchHistory();
        ...
        const interval = setInterval(() => {
            pullFromCloud();
            fetchAllBookings();
        }, 15000); // 15s auto-poll backup

        return () => clearInterval(interval);
    }, []);

    // Live MongoDB-backed Statistics
    ...
    const liveTotalRooms = rooms.length;
    const liveOccupiedRooms = rooms.filter(r => r.status === 'occupied').length;
    const liveAvailableRooms = rooms.filter(r => r.status !== 'occupied' && r.status !== 'maintenance').length;
    const liveBookedRooms = allBookings.filter(b => b.bookingStatus === 'booked' || b.status === 'active' || b.status === 'booked').length;
    const liveTodaysCheckIns = allBookings.filter(b => isToday(b.checkIn)).length;
    const liveTodaysCheckOuts = allBookings.filter(b => isToday(b.checkOut)).length;
    const liveCurrentGuests = allCustomers.filter(c => rooms.some(r => r.tenant === c.name && r.status === 'occupied')).length || liveOccupiedRooms;
    
    const liveMonthlyRevenue = payments
        .filter(p => isCurrentMonth(p.timestamp || p.createdAt))
        .reduce((sum, p) => sum + (p.amount || 0), 0);

    const livePendingPayments = payments
        .filter(p => p.status === 'Waiting for Approval' || p.status === 'pending')
        .reduce((sum, p) => sum + (p.amount || 0), 0);

    const liveStats = [
        { label: 'Total Rooms', value: liveTotalRooms, color: 'blue' },
        { label: 'Occupied Rooms', value: liveOccupiedRooms, color: 'emerald' },
        { label: 'Available Rooms', value: liveAvailableRooms, color: 'indigo' },
        { label: 'Booked Rooms', value: liveBookedRooms, color: 'sky' },
        { label: 'Today\'s Check-ins', value: liveTodaysCheckIns, color: 'amber' },
        { label: 'Today\'s Check-outs', value: liveTodaysCheckOuts, color: 'rose' },
        { label: 'Current Guests', value: liveCurrentGuests, color: 'purple' },
        { label: 'Monthly Revenue', value: `₹${liveMonthlyRevenue.toLocaleString('en-IN')}`, color: 'emerald' },
        { label: 'Pending Payments', value: `₹${livePendingPayments.toLocaleString('en-IN')}`, color: 'amber' },
    ];
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC]">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="flex items-center gap-3">
                                <h1 className="text-3xl font-black italic tracking-tighter uppercase leading-none">Lodge Manager <br /><span className="text-blue-500">Command.</span></h1>
                                <span className="px-3 py-1 bg-blue-500/20 text-blue-400 border border-blue-500/30 text-[9px] font-black uppercase rounded-full tracking-widest">Lodge Manager</span>
                            </div>
                            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-[0.4em] mt-2">Lodge Manager Operations Portal</p>
                        </div>
                    </div>
            ...
            <div className={`relative z-20 px-4 sm:px-6 -mt-12 pb-12 mx-auto min-h-[60vh] ${activeTab === 'payments' || activeTab === 'rooms' || activeTab === 'bills' ? 'max-w-7xl' : 'max-w-6xl'}`}>
                <AnimatePresence mode="wait">
                    ...
                    {activeTab === 'overview' && (
                        ...
                        >
                            ...
                            {/* Live Feeds Grid: Recent Bookings & Recent Payments */}
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                ...
                                <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {allBookings.length === 0 && (
                                            <p className="text-center py-8 text-xs font-bold text-slate-300 uppercase tracking-widest italic">No recent bookings</p>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {payments.length === 0 && (
                                            <p className="text-center py-8 text-xs font-bold text-slate-300 uppercase tracking-widest italic">No transactions recorded</p>
                                        )}
                                    </div>
                                </div>
