...
const UPIFallback = ({ upiData }) => {
  ...
  return (
    <motion.div 
      ...
      className="bg-white dark:bg-slate-900 p-8 rounded-[4rem] flex flex-col items-center gap-8 border-b-[12px] border-blue-600 shadow-2xl relative overflow-hidden"
    >
      <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-blue-500 via-indigo-500 to-blue-500" />
      
      {/* Recipient Header */}
      <div className="w-full flex items-center justify-between pb-6 border-b border-slate-100 dark:border-slate-800">
      ...
      <div className="space-y-4 flex flex-col items-center">
        <div className="p-4 bg-white dark:bg-slate-900 rounded-3xl border-4 border-slate-900 shadow-[8px_8px_0px_0px_#1e293b] group relative">
        ...
        <div className="flex items-center gap-2 text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 dark:bg-slate-800 px-4 py-2 rounded-full border border-slate-100 dark:border-slate-800">
      ...
      <div className="w-full space-y-4">
        <div className="flex items-center gap-4 bg-slate-900 p-2 pl-6 rounded-2xl border border-slate-800">
          ...
          <button 
            ...
            className={`px-6 py-4 rounded-xl flex items-center gap-2 transition-all active:scale-95 ${copied ? 'bg-emerald-500 text-white' : 'bg-blue-600 text-white hover:bg-white dark:bg-slate-900 hover:text-blue-600'}`}
          >
